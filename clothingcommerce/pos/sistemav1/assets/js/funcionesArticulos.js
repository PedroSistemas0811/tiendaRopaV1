...
const funcionesArticulos = {
    showInfoProducto: function(dar) {
        ...
        funcionesArticulos.showCarrusel();
    ...
    updateProducto: function() {
        ...
        if (articuloPosicion !== 'vacio') {
            ...
            callArticulos.editProduct(articuloPosicion);
    ...
    saveImagen: function() {
        ...
        var files = $('#imageArticulo')[0].files[0];
         if (files === null || files === undefined || $('#imageArticulo')[0].files[0] === '') {
            Swal.fire({
                position: 'top-center',
                icon: 'error',
                title: 'Error 404. No se cargo imagen',
                showConfirmButton: true
            })
        } else {
            $.ajax({
    ...
    addNewArticulo : function(){
        limpiarPrincipal();
        $('#guardarArticulo').attr('onclick','funcionesArticulos.addNewArticulo();');
    ...
    $.ajax({
        ...
        error: function() {
             Swal.fire({
                    position: 'top-center',
                    icon: 'error',
                    title: 'Error : Error al recuperar el catálogo.',
                    showConfirmButton: true
                })
        }
    });
